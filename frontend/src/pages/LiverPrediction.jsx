...
const LiverPrediction = () => {
    ...
    const liverFields = [
        ...
        { name: 'Gender', label: 'Gender', range: 'Male, Female', tip: 'Patient gender', options: [{val: 'Male', label: 'Male'}, {val: 'Female', label: 'Female'}] },
        { name: 'Total_Bilirubin', label: 'Total Bilirubin', range: '0-100', tip: 'Total Bilirubin in mg/dL' },
        { name: 'Direct_Bilirubin', label: 'Direct Bilirubin', range: '0-50', tip: 'Direct Bilirubin in mg/dL' },
        { name: 'Alkaline_Phosphotase', label: 'Alkaline Phosphotase', range: '5-3000', tip: 'Alkaline Phosphotase in IU/L' },
        { name: 'Alamine_Aminotransferase', label: 'Alamine Aminotransferase', range: '5-3000', tip: 'Alamine Aminotransferase in IU/L' },
        { name: 'Aspartate_Aminotransferase', label: 'Aspartate Aminotransferase', range: '5-3000', tip: 'Aspartate Aminotransferase in IU/L' },
        { name: 'Total_Protiens', label: 'Total Proteins', range: '1.0-15.0', tip: 'Total Proteins in g/dL' },
        { name: 'Albumin', label: 'Albumin', range: '0.5-10.0', tip: 'Albumin in g/dL' },
        { name: 'Albumin_and_Globulin_Ratio', label: 'A/G Ratio', range: '0.1-5.0', tip: 'Albumin and Globulin Ratio' }
    ];
    ...
    const onSubmit = async (data) => {
        if (!selectedPatient) {
            Swal.fire({ icon: 'warning', title: 'Missing Patient', text: 'Please select a patient before predicting.', confirmButtonColor: '#f97316' });
            return;
        }
        setLoading(true);
        try {
            ...
            Object.keys(formattedData).forEach(k => {
                if(k !== 'Gender') formattedData[k] = parseFloat(formattedData[k]);
            });
            formattedData.patient_id = parseInt(selectedPatient);

            const res = await apiClient.post('/predict/liver', formattedData);
            setResult(res.data);
        } catch (err) {
            ...
            Swal.fire({ icon: 'error', title: 'Invalid Input', html: errorMessage, confirmButtonColor: '#f97316' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDownload = async (path) => {
        ...
        try {
            ...
            Swal.fire({ icon: 'error', title: 'Download Failed', text: 'Could not download the report.', confirmButtonColor: '#f97316' });
        }
    };
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, maxWidth: 1100, margin: '0 auto' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
                <Box sx={{ width: 48, height: 48, borderRadius: '12px', backgroundColor: 'rgba(249, 115, 22, 0.1)', display: 'flex', justifyContent: 'center', alignItems: 'center', boxShadow: 'var(--shadow-sm)' }}>
                    <CircleIcon sx={{ color: '#f97316', fontSize: 28 }} />
                </Box>
                <Box>
                    <Typography variant="h4" fontWeight="700" letterSpacing="-0.02em">Liver Disease Predictor</Typography>
                    <Typography variant="body2" color="text.secondary">Enter hepatology metrics or use AI OCR to predict Liver Disease risk.</Typography>
                </Box>
            </Box>
                            ...
                            <form onSubmit={handleSubmit(onSubmit)}>
                                ...
                                <Box sx={{ mb: 4 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                                        <Typography variant="subtitle2" sx={{ color: '#f97316', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em' }}>STEP 1</Typography>
                                ...
                                {/* Step 2: AI Autofill */}
                                <Box sx={{ mb: 4, p: 3, backgroundColor: 'rgba(249, 115, 22, 0.05)', borderRadius: '12px', border: '1px dashed rgba(249, 115, 22, 0.3)' }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                                        <AutoFixHighIcon sx={{ color: '#f97316' }} />
                                        <Typography variant="h6" fontWeight="700" color="#f97316">Smart OCR Autofill</Typography>
                                    </Box>
                                    <Typography variant="body2" color="text.secondary" mb={2}>
                                        Upload a physical lab report. Our AI will instantly extract the required metrics and populate the form below.
                                    </Typography>
                                    <DocumentUpload diseaseType="Liver Disease" onExtracted={handleExtracted} />
                                </Box>

                                {/* Step 3: Clinical Metrics */}
                                <Box sx={{ mb: 4 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                                        <Typography variant="subtitle2" sx={{ color: '#f97316', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em' }}>STEP 2</Typography>
                                        <Divider sx={{ flexGrow: 1 }} />
                                    </Box>
                                    <Typography variant="h6" fontWeight="700" gutterBottom>Hepatology Metrics</Typography>
