...
const DiabetesPrediction = () => {
    ...
    const onSubmit = async (data) => {
        if (!selectedPatient) {
            Swal.fire({ icon: 'warning', title: 'Missing Patient', text: 'Please select a patient before predicting.', confirmButtonColor: '#2563EB' });
            return;
        }
        setLoading(true);
        try {
            ...
            const res = await apiClient.post('/predict/diabetes', formattedData);
            setResult(res.data);
        } catch (err) {
            console.error(err);
            ...
            Swal.fire({ icon: 'error', title: 'Invalid Input', html: errorMessage, confirmButtonColor: '#2563EB' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDownload = async (path) => {
        ...
        try {
            ...
            Swal.fire({ icon: 'error', title: 'Download Failed', text: 'Could not download the report.', confirmButtonColor: '#2563EB' });
        }
    };
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, maxWidth: 1100, margin: '0 auto' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
                <Box sx={{ width: 48, height: 48, borderRadius: '12px', backgroundColor: 'rgba(37, 99, 235, 0.1)', display: 'flex', justifyContent: 'center', alignItems: 'center', boxShadow: 'var(--shadow-sm)' }}>
                    <WaterDropIcon sx={{ color: '#2563EB', fontSize: 28 }} />
                </Box>
                <Box>
                    <Typography variant="h4" fontWeight="700" letterSpacing="-0.02em">Diabetes Predictor</Typography>
                    <Typography variant="body2" color="text.secondary">Enter clinical metrics or use AI OCR to predict Type II Diabetes risk.</Typography>
                </Box>
            </Box>
                            ...
                            <form onSubmit={handleSubmit(onSubmit)}>
                                ...
                                <Box sx={{ mb: 4 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                                        <Typography variant="subtitle2" color="primary.main" fontWeight="700" sx={{ textTransform: 'uppercase', letterSpacing: '0.05em' }}>STEP 1</Typography>
                                ...
                                {/* Step 2: AI Autofill */}
                                <Box sx={{ mb: 4, p: 3, backgroundColor: 'rgba(6,182,212,0.05)', borderRadius: '12px', border: '1px dashed rgba(6,182,212,0.3)' }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                                        <AutoFixHighIcon sx={{ color: '#06B6D4' }} />
                                        <Typography variant="h6" fontWeight="700" color="#06B6D4">Smart OCR Autofill</Typography>
                                    </Box>
                                    <Typography variant="body2" color="text.secondary" mb={2}>
                                        Upload a physical lab report. Our AI will instantly extract the required metrics and populate the form below.
                                    </Typography>
                                    <DocumentUpload diseaseType="Diabetes" onExtracted={handleExtracted} />
                                </Box>

                                {/* Step 3: Clinical Metrics */}
                                <Box sx={{ mb: 4 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                                        <Typography variant="subtitle2" color="primary.main" fontWeight="700" sx={{ textTransform: 'uppercase', letterSpacing: '0.05em' }}>STEP 2</Typography>
                                        <Divider sx={{ flexGrow: 1 }} />
                                    </Box>
                                    <Typography variant="h6" fontWeight="700" gutterBottom>Clinical Metrics</Typography>
                                    <Grid container spacing={3} sx={{ mt: 1 }}>
                                        {[
                                            { name: 'Pregnancies', label: 'Pregnancies', range: '0-20', tip: 'Number of times pregnant' },
                                            { name: 'Glucose', label: 'Fasting Glucose', range: '0-400', tip: 'Plasma glucose concentration (mg/dL)' },
                                            { name: 'BloodPressure', label: 'Blood Pressure', range: '0-200', tip: 'Diastolic blood pressure (mm Hg)' },
                                            { name: 'SkinThickness', label: 'Skin Thickness', range: '0-100', tip: 'Triceps skin fold thickness (mm)' },
                                            { name: 'Insulin', label: 'Insulin Level', range: '0-1000', tip: '2-Hour serum insulin (mu U/ml)' },
                                            { name: 'BMI', label: 'Body Mass Index', range: '0-70', tip: 'Weight in kg/(height in m)^2' },
                                            { name: 'DiabetesPedigreeFunction', label: 'Pedigree Function', range: '0.0-3.0', tip: 'Diabetes pedigree function' },
